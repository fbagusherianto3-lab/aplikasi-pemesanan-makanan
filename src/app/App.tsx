...
function MainApp({
  ...
  // Modal State Edit Pesanan Dapur
  ...
  function handleSaveEditOrder() {
    ...
    // Recalculate total
    const newTotal = editingOrder.items.reduce((s, item) => s + item.price * item.qty, 0);
    const updatedOrder = { ...editingOrder, total: newTotal };

    setOrders((prev) => prev.map((o) => (o.id === updatedOrder.id ? updatedOrder : o)));
    setEditOrderModalOpen(false);
    setEditingOrder(null);
    showToast(`Pesanan ${updatedOrder.id} berhasil diperbarui! ✏️`);
  }
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      {/* Modal Edit Pesanan Dapur */}
      {editOrderModalOpen && editingOrder && (
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4">
          <div className="bg-card w-full max-w-md rounded-2xl p-5 border border-border shadow-2xl relative">
            <button onClick={() => setEditOrderModalOpen(false)} className="absolute top-4 right-4 text-muted-foreground hover:text-foreground">
              <X size={18} />
            </button>
            <h3 className="text-lg font-bold text-foreground mb-4 flex items-center gap-2">
              <Pencil size={18} className="text-primary" />
              <span>Edit Pesanan ({editingOrder.id})</span>
            </h3>

            <div className="space-y-3 max-h-96 overflow-y-auto pr-1">
              <div>
                <label className="block text-xs font-semibold mb-1">Nomor Meja</label>
                <select
                  value={editingOrder.tableNo}
                  onChange={(e) => setEditingOrder({ ...editingOrder, tableNo: e.target.value })}
                  className="w-full px-3 py-2 rounded-lg bg-input-background border border-border text-xs focus:outline-none"
                >
                  {ALL_TABLES.map((t) => (
              ...
              <div>
                <label className="block text-xs font-semibold mb-1">Status Pesanan</label>
                <select
              ...
              <div>
                <label className="block text-xs font-semibold mb-1">Item Pesanan</label>
                <div className="space-y-2 bg-secondary p-2.5 rounded-xl">
                    ...
                    <div key={item.id} className="flex items-center justify-between text-xs">
                      ...
                      <div className="flex items-center gap-1">
                        <button
                          ...
                          className="w-5 h-5 rounded bg-card border border-border flex items-center justify-center font-bold"
                        >
                          <Minus size={10} />
                        </button>
                        ...
                        <button
                          ...
                          className="w-5 h-5 rounded bg-primary text-white flex items-center justify-center font-bold"
                        >
                          <Plus size={10} />
                        </button>
                      </div>
                  ...
                  {editingOrder.items.length === 0 && (
                    <p className="text-[11px] text-muted-foreground text-center py-2">Tidak ada item dalam pesanan</p>
                  )}
                </div>
              </div>
            </div>
            ...
            <div className="flex gap-2 mt-5">
              <button onClick={() => setEditOrderModalOpen(false)} className="flex-1 bg-secondary text-foreground text-xs font-bold py-2 rounded-lg">
                Batal
              </button>
              <button onClick={handleSaveEditOrder} className="flex-1 bg-primary text-white text-xs font-bold py-2 rounded-lg hover:bg-red-700">
                Simpan
              </button>
            </div>
          </div>
        ...
        <div className="fixed inset-0 z-50 flex justify-end bg-black/40">
          <div className="w-full max-w-sm bg-card h-full flex flex-col shadow-2xl">
            <div className="p-4 bg-primary text-white flex justify-between items-center">
              ...
              <button onClick={() => setPanel("none")} className="p-1 hover:bg-white/10 rounded-full">
                <X size={18} />
              </button>
            </div>
          ...
          <div className="w-full max-w-md bg-card h-full flex flex-col shadow-2xl">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-3">
              {orders.length === 0 ? (
                ...
                orders.map((o) => (
                  <div key={o.id} className="border border-border bg-secondary p-3 rounded-xl space-y-2 relative">
                    ...
                    {o.note && (
                      <p className="text-[11px] text-muted-foreground italic bg-card/60 px-2 py-1 rounded border border-border/50">
                        Catatan: {o.note}
                      </p>
                    )}
                    ...
                    <div className="flex justify-between items-center pt-2 border-t border-border font-bold text-xs">
                      ...
                      <div className="flex items-center gap-1">
                        {o.status === "Menunggu" && (
                          <button onClick={() => updateOrderStatus(o.id, "Diproses")} className="bg-blue-600 hover:bg-blue-700 text-white px-2.5 py-1 rounded text-[10px] font-semibold">
                            Proses
                          </button>
                        )}
                        {o.status === "Diproses" && (
                          <button onClick={() => updateOrderStatus(o.id, "Selesai")} className="bg-green-600 hover:bg-green-700 text-white px-2.5 py-1 rounded text-[10px] font-semibold">
                            Selesai
                          </button>
                        )}
                      </div>
